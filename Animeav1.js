class DefaultExtension extends MProvider {
    ...
    removeAccents(text) {
        ...
        // Colapsar espacios horizontales pero MANTENER saltos de línea
    ...
    async getDetail(url) {
        ...
        var relTypes = { 1: "Precuela", 2: "Secuela", 3: "Historia Principal", 4: "Relacionado", 5: "Alternativo", 6: "Spin-off", 7: "Adaptacion", 8: "Resumen", 9: "Otro", 10: "Especial/Peli", 11: "Historia Paralela" };
        var relInfo = "";
        ...
        if (relationsContent) {
            ...
            while ((rMatch = rEntryRegex.exec(relationsContent)) !== null) {
                var tNum = parseInt(rMatch[1]);
                var chunk = rMatch[2];
                var rSlug = getVal(chunk, "slug");
                var rTitle = getVal(chunk, "title");
                
                if (rSlug && rSlug !== currentSlug) {
                    relations.push({ type: tNum, slug: rSlug, title: rTitle });
                }
            }

            // Ordenar por tipo para mantener consistencia
            relations.sort((a, b) => a.type - b.type);

            for (var i = 0; i < relations.length; i++) {
                var rel = relations[i];
                var label = relTypes[rel.type] || "Relacionado";
                relInfo += "* " + label + ": " + this.removeAccents(rel.title.replace(/[\[\]]/g, "")) + "\n";
            }
        }
